...
<Tabs syncKey="option">
  <TabItem label="GitHub">
    ...
    <Steps>
      1. The option `Repository destination` points by default to the current :term[repository] you are working on.

      2. You can leave the option `Branch source` set to `main`, unless you have a specific reason to start from a different branch.

      3. Keep the option `:term[Checkout] locally` selected.
  ...
  <TabItem label="GitLab">
    ...
    <Steps>
      1. You can leave the option `Branch source` set to `main`, unless you have a specific reason to start from a different branch.

      2. You can edit the suggested branch name, if needed.
    </Steps>
  </TabItem>
</Tabs>
